<template>
  <div class="bg-gray-100">
    <div class="bg-white pb-8 text-center pt-24 text-3xl leading-8 font-extrabold tracking-tight text-gray-900 pl-24 pr-24 sm:text-4xl d-flex justify-content-between">
      {{ route.params.id ? model.title : "Create a Virtual Lab" }}
      <button
...
function saveVlab() {
  store.dispatch("saveVlab", model.value).then(({ data }) => {
    store.commit('notify', {
      type: 'success',
      message: 'Vlab was successfully updated'
    })
    router.push({
      name: "VlabsView",
...
function deleteVlab() {
  if (
    confirm (
      `Are you sure you want to delete this Vlab? Operation can't be undone.`
    )
  ) {
