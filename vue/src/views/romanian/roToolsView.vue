<template>
  <div class="bg-gray-100">
    <div class="bg-white pb-8 text-center pt-24 text-3xl leading-8 font-extrabold tracking-tight text-gray-900 pl-24 pr-24 sm:text-4xl d-flex justify-content-between">
      {{ route.params.id ? model.title : "Creeaza un instrument" }}
      <button
      ...
      <div class="sm:rounded-md sm:overflow-hidden pr-12 pl-12 mt-12 pb-24">
        ...
        <div class="px-4 py-5 bg-white space-y-6 sm:p-6 mr-36 ml-36">
          ...
          <div>
            ...
            <div class="mt-1">
              <textarea
                ...
                placeholder="Descrie instrumentul de calcul"
              />
            </div>
          </div>
          <!-- Description -->
          ...
          <div>
            ...
            <div>Selectează formula</div>
            <select v-model="model.formula">
              <option>Punte Wheatstone</option>
              <option>Rezistenta Echivalenta in Paralel</option>
              <option>Legile Faraday Lenz</option>
              <option>Unitati de masura Presiune</option>
              <option>Calculator Energii</option>
              <option>Constanta de timp circuit RC</option>
            </select>
          </div>
...
function saveTool() {
  store.dispatch("saveTool", model.value).then(({ data }) => {
    store.commit('notify', {
      type: 'success',
      message: 'Instrumentul a fost actualizat cu succes!'
    })
    router.push({
      name: "VeziInstrumente",
...
function deleteTool() {
  if (
    confirm (
      `Ești sigur că vrei să ștergi acest Instrument?.`
    )
  ) {
